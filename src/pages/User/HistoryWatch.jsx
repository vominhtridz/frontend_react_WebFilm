...
const HistoryWatch = () => {
  ...
  const username = localStorage.getItem("user")
    ...
    : "Người dùng"
  const storageFilm = Cookies.get("history")
  useEffect(() => {
    const fetchFilms = async () => {
      const arrFilm = Cookies.get("history") ? JSON.parse(Cookies.get("history")) : []
      if (arrFilm.length > 0) {
        try {
  ...
  const handleRemoveFilm = path => {
    const ListPathFilm = JSON.parse(Cookies.get("history"))
    const newPathFilms = ListPathFilm.filter(film => film !== path)
    Cookies.set("history", JSON.stringify(newPathFilms))
  }
  return (
    <div className='w-full bg-gray-600 p-6 min-h-[50vh] rounded-sm shadow '>
      ...
      {Films.length > 0 ? (
        ...
        <h2 className='text-2xl text-center pt-14 text-white font-medium'>
          Bạn chưa Xem Phim
          <Link className='text-blue-500 px-2 hover:text-blue-600 hover:underline' to='/'>
            Trang chủ
          </Link>
        </h2>
      )}
